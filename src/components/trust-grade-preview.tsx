...
export function TrustGradePreview({
  businessName,
  onComplete,
}: TrustGradePreviewProps) {
  const trustMetrics = [
    {
  ...
  const overallScore = Math.round(
    trustMetrics.reduce((sum, metric) => sum + metric.score, 0) /
      trustMetrics.length
  );
  const trustGrade =
    overallScore >= 90
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "fair":
        return <Badge className="bg-yellow-500/20 text-yellow-400">Fair</Badge>;
      case "needs-improvement":
        return <Badge className="bg-red-500/20 text-red-400">Needs Work</Badge>;
      default:
        ...
        <CardContent className="p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400">
              Here&apos;s how your business profile looks to potential partners
            </p>
          </div>
          ...
          <div className="flex flex-col lg:flex-row items-center justify-center space-y-6 lg:space-y-0 lg:space-x-12">
            ...
            <div className="text-center lg:text-left">
              ...
              <p className="text-gray-300 max-w-md">
                ...
                !
                {overallScore < 80 &&
                  " Complete the remaining steps to improve your trust score."}
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card className="bg-gradient-to-br from-green-600 to-green-700 border-green-500">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-green-100 mb-4">
            Your business profile is now live. Start connecting with verified
            partners and building trust in your network.
          </p>
          <button
            onClick={onComplete}
